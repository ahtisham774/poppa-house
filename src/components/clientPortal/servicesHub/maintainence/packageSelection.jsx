...
const PackageSelection = ({ service, onSelectPackage, onBack }) => {
  ...
  return (
    <div className="p-3 lg:p-8 w-full">
      ...
      <div className="mt-7 flex gap-4 justify-between flex-wrap-reverse">
        <button 
          className="border w-full sm:w-fit rounded-lg px-7 py-2 text-[#131e47] font-medium"
          onClick={onBack}
        >
          Previous
        </button>
        <button
          className="bg-[#131e47] w-full sm:w-fit rounded-lg px-7 py-2 text-white font-medium"
          onClick={() => {
            if (selectedPackage) {
