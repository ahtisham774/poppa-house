...
// };
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      try {
        setLoading(true)
        const response = await clientDocumentService.getDocuments(user?.id)
        const documents = response || []

        // Map uploaded documents to our state
        const updatedCategories = documentCategories.map(category => {
          const updatedDocuments = category.documents.map(doc => {
            ...
            if (uploadedDoc) {
              ...
              return {
                ...doc,
                status: uploadedDoc.verified ? 'verified' : 'pending',
                fileName: uploadedDoc.url.split('/').pop(),
                submittedDate: new Date(
                  uploadedDoc.uploadedAt._seconds * 1000
                ).toLocaleDateString('en-US', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
                }),
                verifiedDate: uploadedDoc.verified
                  ? new Date(
                      uploadedDoc.uploadedAt._seconds * 1000
                    ).toLocaleDateString('en-US', {
                      year: 'numeric',
        ...
        // Set uploaded files for the table
        const files = documents.map(doc => ({
          id: doc.id,
          fileName: doc.url.split('/').pop(),
          fileType: doc.url.match(/\.(jpg|jpeg|png|gif)$/i)
            ? 'Image'
            : doc.url.match(/\.(pdf)$/i)
            ? 'Document'
            : doc.url.match(/\.(mp4|mov|avi)$/i)
            ? 'Video'
            : 'Document',
          fileSize: 'N/A', // You might need to get this from Cloudinary response
          uploadedDate: new Date(
            doc.uploadedAt._seconds * 1000
          ).toLocaleDateString('en-US', {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
  ...
  const handleDocumentUpload = async (documentId, file) => {
    try {
      setLoading(true)
      const documentType = documentCategories
        .flatMap(cat => cat.documents)
        .find(doc => doc.id === documentId)?.type
      ...
      clientDocumentService
        .uploadDocument(user?.id, file, documentType)
        .then(response => {
          showToast('success', response.message || 'Document uploaded successfully')
  ...
  const renderDocumentCard = document => {
    ...
    return (
      ...
      >
        ...
        <p className='text-sm text-[#888888] w-full my-1'>
        ...
        {documentCategories.map(category => (
          <div key={category.id} className='mb-1'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-5'>
              {category.documents.map(document => renderDocumentCard(document))}
            </div>
          </div>
        ))}
      </div>
